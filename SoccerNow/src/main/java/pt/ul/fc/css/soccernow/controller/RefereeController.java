...
public class RefereeController {
    ...
    public ResponseEntity<List<RefereeDTO>> getAllReferees(@ModelAttribute RefereeSearchParams params,
        ...
        Stream<Referee> refereeStream = refereeService.findAllNotDeleted(params).stream();
        if (optionalRefereeComparator.isPresent()) {
            refereeStream = refereeStream.sorted(optionalRefereeComparator.get());
        }

        return ResponseEntity.ok(refereeStream.map(refereeMapper::toDTO).toList());
    }
